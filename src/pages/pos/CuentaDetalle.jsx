...
import {
  ...
  AlertTriangle, Printer, ChefHat, Scissors
} from 'lucide-react'
...
export default function CuentaDetalle() {
  ...
  async function fetchAll() {
    ...
    // Marcar qué productos tienen modificadores
    const { data: assignments } = await supabase
      .from('product_modifier_group_assignments')
      .select('product_id')
    const hasMods = new Set((assignments ?? []).map(a => a.product_id))
    setProducts((prodRes.data ?? []).map(p => ({ ...p, hasMods: hasMods.has(p.id) })))
  ...
  const addToCart = useCallback((product, selectedMods = [], fromModal = false, comboItems = []) => {
    ...
    setCart(prev => {
      ...
      return [...prev, { cartKey, product_id: product.id, name: product.name, price: finalPrice, mods: selectedMods, comboItems, qty: 1 }]
    })
  }, [])
  ...
  function printOrder() {
    ...
    const label = account?.table_name ?? `Cuenta ${account?.id?.slice(0, 4).toUpperCase()}`
    ...
    <h2>${label}</h2><p class="sub">Orden · ${hora}</p>
  ...
  async function sendComanda() {
    ...
    setSendingCmd(false)
    if (error) {
      alert('Error comanda: ' + error.message)
      return
    }
    setCmdSent(true)
...
function ModifierModal({ product, onClose, onConfirm }) {
  ...
  function toggle(group, mod) {
    setSelected(prev => {
      ...
      if (group.multi_select) { cur.has(mod.id) ? cur.delete(mod.id) : cur.add(mod.id) }
  ...
  function getModObj(id) {
    for (const g of groups) { const m = g.modifiers?.find(x => x.id === id); if (m) return m }
    return null
  }
  ...
  function handleConfirm() {
    ...
    for (const g of groups) {
      ...
      if (g.required && selIds.size === 0) { alert(`Debes elegir una opción en "${g.name}"`); return }
      for (const id of selIds) { const mod = getModObj(id); if (mod) allMods.push(mod) }
    }
    onConfirm(allMods, comboItems)
  }

  const extraTotal = Object.values(selected).flatMap(s => [...s])
...
function PaymentModal({ total, onClose, onComplete }) {
  ...
  async function handleConfirm() {
    ...
    if (method === 'mixto') {
      ...
      catch (err) { setError('Error: ' + (err?.message ?? JSON.stringify(err))); setLoading(false) }
      return
    }
    if (!validCash) { setError('El efectivo es menor al total'); return }
    ...
    <div className="fixed inset-0 z-50 bg-black/60 flex items-center justify-center p-4">
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-sm max-h-[90vh] overflow-y-auto">
        ...
        <div className="p-5 space-y-4">
          ...
          {method === 'mixto' && (
            <div className="space-y-2">
              <p className="text-xs text-gray-500">Distribuye el total entre los métodos que uses</p>
              {[
