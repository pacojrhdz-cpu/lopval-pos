...
export default function Requisition() {
  ...
  async function fetchStock() {
    ...
    const sm = {}
    for (const r of (data ?? [])) sm[r.ingredient_id] = r.quantity
    setStockMap(sm)
  }
          ...
          {filteredIngs.map(ing => {
            ...
            return (
              ...
              >
                ...
                {stock != null && (
                  <p className={`text-xs font-medium mt-1 ${lowStock ? 'text-amber-600' : 'text-emerald-600'}`}>
                    {lowStock ? '⚠ ' : ''}{stock} {ing.unit}
                  </p>
                )}
                {isSel && (
