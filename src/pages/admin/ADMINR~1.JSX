...
export default function AdminRequisitions() {
  ...
  async function fetchAll() {
    ...
    let q = supabase
      .from('requisitions')
      .select('*, requisition_items(id, ingredient_id, ingredient_name, unit, quantity_requested, approved_qty, received_qty, notes)')
  ...
  async function cancel(id) {
    setSaving(true)
    await supabase.from('requisitions').update({ status: 'rechazada' }).eq('id', id)
    setReviewing(null); setReviewNote(''); setSaving(false); fetchAll()
  }
