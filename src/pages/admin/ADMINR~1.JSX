...
import {
  ...
  CheckCircle, XCircle, Clock, Printer, Package
} from 'lucide-react'
...
async function printRequisition(req, logoUrl = null) {
  ...
  const STATUS_TEXT  = { pending: 'PENDIENTE', approved: 'APROBADA', received: 'RECIBIDA', cancelled: 'CANCELADA', partial: 'PARCIAL' }
  const STATUS_COLOR = { pending: '#f59e0b', approved: '#22c55e', received: '#3b82f6', cancelled: '#ef4444', partial: '#f97316' }
  ...
  const itemRows = (req.requisition_items ?? []).map((ri, idx) => `
    <tr style="background:${idx % 2 === 0 ? '#fff' : '#f9fafb'}">
      ...
      <td style="padding:9px 14px;border-bottom:1px solid #e5e7eb;text-align:right;font-weight:700;font-size:12px">${ri.requested_qty}</td>
  ...
  const body = `
    ...
    <div style="background:#f3f4f6;padding:14px 32px;display:flex;border-bottom:2px solid #e5e7eb">
      ...
      <div style="flex:1.5;padding-right:24px;margin-right:24px;border-right:1px solid #d1d5db">
        ...
        <div style="font-size:12px;font-weight:600">${req.created_by_name ?? '—'}</div>
      </div>
      <div style="flex:1">
    ...
    <div style="padding:24px 32px">
      ...
      <div style="margin-top:52px;display:flex;gap:48px">
        <div style="flex:1"><div style="border-top:1.5px solid #111;padding-top:8px">
          <div style="font-size:11px;font-weight:700">${req.created_by_name ?? 'Solicitante'}</div>
...
const STATUS_LABEL = {
  ...
  received:  { label: 'Recibida',  cls: 'bg-blue-100 text-blue-700' },
...
export default function AdminRequisitions() {
  ...
  async function fetchAll() {
    ...
    let q = supabase
      .from('requisitions')
      .select('*, requisition_items(id, ingredient_id, ingredient_name, unit, requested_qty, approved_qty, received_qty, notes)')
  ...
  async function approve(req) {
    ...
    await supabase.from('requisitions').update({
      ...
      notes:            reviewNote || req.notes || null,
    }).eq('id', req.id)
  ...
  async function cancel(id) {
    setSaving(true)
    await supabase.from('requisitions').update({ status: 'cancelled' }).eq('id', id)
    setReviewing(null); setReviewNote(''); setSaving(false); fetchAll()
  }
                ...
                <button
                  ...
                  <div className="flex-1">
                    <p className="text-sm font-semibold text-gray-800">
                      {req.branch_name ?? 'Sucursal'} — {req.created_by_name ?? 'Usuario'}
                    </p>
                      ...
                      <table className="w-full text-sm">
                        ...
                        <tbody className="divide-y divide-gray-50">
                          {req.requisition_items?.map(ri => {
                            ...
                            const isLow = currentStock != null && currentStock < ri.requested_qty
                            return (
                          ...
                          <>
                            ...
                            <div className="flex gap-2">
                              <button onClick={() => cancel(req.id)} disabled={saving}
                                ...
                                <XCircle className="w-4 h-4" /> Cancelar requi
                              </button>
