...
function AdminReceiveModal({ req, onClose, onDone }) {
  ...
  async function handleConfirm() {
    ...
    // Actualizar received_qty en cada item y sumar al inventario de la sucursal
    for (const ri of items) {
      const r = received[ri.id]
      const receivedQty = parseFloat(r?.qty) || 0

      // 1. Guardar cantidad recibida en el item (columna opcional)
      await supabase.from('requisition_items')
        .update({ received_qty: receivedQty })
        .eq('id', ri.id)

      // 2. Sumar al inventario de la sucursal destino usando RPC update_stock
      if (receivedQty > 0 && req.branch_id && ri.ingredient_id) {
        const { error: stockErr } = await supabase.rpc('update_stock', {
          p_branch_id:     req.branch_id,
          p_ingredient_id: ri.ingredient_id,
          p_delta:         receivedQty,
          p_type:          'requisition_receipt',
          p_notes:         `Requi #${req.id.slice(0,6).toUpperCase()}`,
          p_user_id:       user?.id ?? null,
          p_user_name:     profile?.name ?? 'Admin',
        })
        if (stockErr) console.error('[AdminReceive] update_stock error:', stockErr.message, ri.ingredient_name)
      }
    }
    ...
    const { error: reqErr } = await supabase.from('requisitions').update({
      status:      'recibida',
      review_note: summary,
    }).eq('id', req.id)

    if (reqErr) {
      setErr('Error al actualizar estado: ' + reqErr.message)
      setSaving(false)
      return
    }
