...
export default function Inventory() {
  ...
  async function fetchAll() {
    let ingQ = supabase.from('ingredients').select('*').eq('active', true).order('name')
    if (activeBranch?.id) ingQ = ingQ.eq('branch_id', activeBranch.id)

    let movQ = supabase.from('inventory_movements')
      .select('*, ingredients(name), profiles(name)')
      .order('created_at', { ascending: false }).limit(50)
    if (activeBranch?.id) movQ = movQ.eq('branch_id', activeBranch.id)

    const [{ data: ing }, { data: mov }] = await Promise.all([ingQ, movQ])
    setIngredients(ing ?? [])
    setMovements(mov ?? [])
    setLoading(false)
  }
  ...
  async function saveIngredient() {
    ...
    if (editIngr.id) {
      ...
      await supabase.from('ingredients').insert({ ...payload, stock_quantity: 0, active: true, branch_id: activeBranch?.id ?? null })
    }
    setEditIngr(null)
    setSaving(false)
    fetchAll()
